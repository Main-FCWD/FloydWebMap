<script>
	...
	onMount(async () => {
		if (browser) {
			...
			map = L.map(mapElement, { zoomControl: true, maxZoom: 35, minZoom: 1 }).setView(
			...
			var layer_OSM_dark = L.tileLayer('http://tile.openstreetmap.org/{z}/{x}/{y}.png', {
				className: 'map-tiles',
				pane: 'pane_OSM_dark',
				opacity: 1.0,
				minZoom: 1,
			...
			function addMarkersToMap(map, meters) {
				...
				meters.forEach((meter) => {
					...
					const markerIcon = L.icon({
						iconUrl: `src/Data/markers/rt${Route}.svg`,
				...
				var CollapseBtn = `<div><img class="layerControlToggle" style="float:right;" src="src/Data/images/lct_button.png"></div>`;
